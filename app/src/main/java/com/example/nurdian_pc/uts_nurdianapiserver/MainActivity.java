package com.example.nurdian_pc.uts_nurdianapiserver;
...
public class MainActivity extends AppCompatActivity {
    ...
    private class Getcontacts extends AsyncTask<Void, Void, Void> {
        ...
        public void onClick(View view, int position) {
            MoveWithObject moveWithObject= MainActivity.get(position);
            Toast.makeText(getApplicationContext(), moveWithObject.getKota() + " is selected ",Toast.LENGTH_SHORT).show();
            Intent intent = new Intent(MainActivity.this, MoveWithObject.class);
            intent.putExtra("kota",moveWithObject.getKota());
            intent.putExtra("tanggal",moveWithObject.getTanggal());
            intent.putExtra("waktusholat",moveWithObject.getWaktusholat());
            startActivity(intent);
        }

    }

    private static MoveWithObject get(int position) {
        return null;
    }
}
